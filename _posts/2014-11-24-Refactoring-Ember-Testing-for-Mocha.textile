...
h2. Extraction of Ember Test Helpers
...
Ember Test Helpers is a test-framework-agnostic set of helpers for unit testing Ember applications. Its primary purpose is to provide classes that represent testing modules - a common concept across test frameworks. There's a base class for general purpose unit testing (<code>TestModule</code>), as well as derived classes with custom logic for testing components (<code>TestModuleForComponent</code>) and models (<code>TestModuleForModel</code>).

These modules provide a container for testing a particular "unit" of your application in isolation. The modules set up the context of each test, providing helpers to access the subject of the test (<code>this.subject()</code>) and other dependencies (such as a store for model tests).

h2. Ember QUnit and Ember Mocha
...
"Ember CLI Mocha":https://github.com/switchfly/ember-cli-mocha contains Mocha-specific blueprints that replace all of the default tests in Ember CLI. After installation, all the standard generators (e.g. <code>ember generate component x-foo</code>) will create tests for Mocha instead of QUnit.
